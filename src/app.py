...
TG_MODELS = ["openai-community/gpt2"]  # Models for Text Generation
...
with st.sidebar:
    ...
    st.text_input(
        ...
        type="password",
        key="openai_api_key",
        help="Please enter your OpenAI API key to use TELL-ME.",
    )
    ...
    st.info(
        "This is a publicly hosted demo running on a free-tier server. "
        "Performance may be limited, and usage restrictions may apply. "
        "For better speed, reliability, and security, we recommend deploying this prototype on your own infrastructure."
    )
...
utils = Utils()
chatgpt = ChatGPT(st.session_state.openai_api_key)
xai = XAI(st.session_state.openai_api_key, model, tokenizer)
...
with HomeTab:
    ...
    if not st.session_state.openai_api_key:
        st.warning("You must enter your OpenAI API key to access this section.")
    else:
